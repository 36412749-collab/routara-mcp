    ...
    if (err instanceof RoutaraApiError) {
        return {
            ...
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({ error: err.message, status: err.status, code: err.code }, null, 2),
                },
            ],
        };
    }
...
async function main() {
    ...
    server.tool('routara_list_models', 'List available LLM and media models on Routara (OpenAI-compatible catalog).', {}, async () => {
        try {
    ...
    server.tool('routara_chat', 'Send a chat completion via Routara (same as OpenAI POST /v1/chat/completions).', {
    ...
    server.tool('routara_generate_image', 'Generate an image via Routara (POST /v1/images/generations). Requires cash wallet balance.', {
    ...
    server.tool('routara_generate_video', 'Submit async video generation (POST /v1/videos/generations). Requires cash balance.', {
    ...
    server.tool('routara_get_video_status', 'Poll video generation task status (GET /v1/videos/:id).', {
